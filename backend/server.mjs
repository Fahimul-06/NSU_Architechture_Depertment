...
async function validateArrival(a,{deviceName='Faculty Office Scanner',method='QR'}={}){
  ...
  await a.save();
  await ScanLog.create({id:crypto.randomUUID(),appointmentId:a.id,token:a.token,result:'ACCEPTED',reason:'VALID',deviceName,scannedAt:now,method});
  return{accepted:true,reason:'VALID',message:'Appointment accepted. Please wait for the faculty response.',appointment:appointmentDetails(a),scannedAt:now.toISOString()};
}
